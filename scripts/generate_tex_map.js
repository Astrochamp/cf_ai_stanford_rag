...
function createTexToUnicodeMap() {
  const translationMap = {};
  for (const group in symbols) {
    for (const texCommand in symbols[group]) {
      const properties = symbols[group][texCommand];
      const unicodeChar = properties.replace || properties.text;
      if (unicodeChar && typeof unicodeChar === 'string') {
        // remove leading \ from tex command
        const cleanCommand = texCommand.substring(1);
        translationMap[cleanCommand] = unicodeChar;
      }
    }
  }
...
const simpleMap = createTexToUnicodeMap();
// create data directory if it doesn't exist
if (!fs.existsSync('data')) {
  fs.mkdirSync('data');
}
fs.writeFileSync('data/tex-unicode-map.json', JSON.stringify(simpleMap, null, 2));

console.log(`✅ Successfully created tex-unicode-map.json with ${Object.keys(simpleMap).length} symbols.`);