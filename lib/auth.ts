 ...
 */
export async function generateWorkerAuthToken(audience: string, privateKeyPem: string): Promise<string> {
  if (!audience) {
 ...
 */
export async function verifyWorkerAuthToken(token: string, workerPublicKeyPem: string, expressServerUrl: string): Promise<jose.JWTVerifyResult | null> {
  try {
...
/**
 * Creates an Express middleware to verify Worker authentication
 * Usage: 
 *   const verifyWorkerAuth = createVerifyWorkerAuth(workerPublicKeyPem, expressServerUrl);
 *   app.post('/endpoint', verifyWorkerAuth, (req, res) => { ... })
 */
export function createVerifyWorkerAuth(workerPublicKeyPem: string, expressServerUrl: string) {
  return async function verifyWorkerAuth(req: any, res: any, next: any) {
    try {
      ...
      const verified = await verifyWorkerAuthToken(token, workerPublicKeyPem, expressServerUrl);
